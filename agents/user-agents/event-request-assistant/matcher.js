...
class OrganizerMatcher {
  ...
  calculateMatchScore(organizer, entities) {
    ...
    if (organizer.expertise && entities.eventType) {
      const expertiseMatch = organizer.expertise.some(
        (exp) =>
    ...
    if (
      ...
      const locationMatch = entities.locations.some(
        (loc) =>
    ...
    if (organizer.priceRange && entities.budget) {
      ...
      if (entities.budget >= min && entities.budget <= max) {
        ...
        analysis.considerations.push(
          `Budget is below organizer's minimum ($${min})`
        );
      } else {
        analysis.considerations.push(
          `Budget exceeds organizer's typical range`
        );
      }
    }
...
module.exports = {
  ...
  getMatchAnalysis: (organizer, entities) =>
    matcher.getMatchAnalysis(organizer, entities),
  OrganizerMatcher,
};
